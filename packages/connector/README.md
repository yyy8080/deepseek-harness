...
Both packages above reach a target the operator configured in advance. A deployment that instead wants a user to enroll their own machine from the browser mounts [`dsh-host-connector-portal`](../host/connector-portal/README.md): it serves a generated start script, the agent dials back over an HTTP upgrade in `dsh-connector-host`'s attach mode, and the machine behind it joins this same registry.

The [connector execution-world decision](../../.agents/notes/implemented/architecture/2026-08-26-connector-execution-world.md) records why the binding is a session event, why the target side reuses the shipped local providers, and which alternatives were rejected; the [connector download portal decision](../../.agents/notes/implemented/architecture/2026-08-26-connector-download-portal.md) records the reversed attachment and the one-package portal.
