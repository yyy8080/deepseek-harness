...
`session.create` 还接受 `connectorId`，它通过追加单个 `connector/bound` 事件，把新对话的执行世界绑定到一个已注册的[连接器](../../connector/connector/README.zh.md)。注册项在创建任何东西之前就被核对，因此未注册的 id 会以 `connector-not-registered` 作答且不留下任何会话——让对话在 UI 承诺了另一台机器的情况下悄悄跑在 harness 机器上，正是这道检查要防的失败。指名连接器同时也声明 `cwd` 是目标机器文件系统里的路径：网关原样记录它，既不在本机创建也不校验。真正抵达目标仍需要一个组合了连接器版 provider 的 agent preset，浏览器从 `connectorPortal.list` 而非本领域得知这一点。创建结果、每一行已接入的 `session.list` 以及 `host/session-added` 都会报出解析出的 `connectorId`，客户端据此标注这场对话，并据此知道它不属于任何本地工作区。它从日志——绑定唯一的权威——解析而来，因此由会话头索引投影、不读日志的冷会话行不带这个字段，直到该会话被接入。
