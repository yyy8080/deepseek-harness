...
| [`connector/`](connector/README.md) | Connector capability family: registry seam, transports, off-box fs/subprocess providers | Product — stable API |
...
| [`marketplace/`](marketplace/README.md) | Plugin catalog seam, static index provider, profile install with provenance | Product — stable API |
...
**Extension plugins depend on Service Definitions, never concrete providers.** `dsh-agent-loop` is swappable; UI, hook, and tool plugins use `dsh-agent`. Composition bundles, including `dsh-agent-spine-demo`, may depend on spine plugins. Role separation follows [capability seams](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md).

Package READMEs cover purpose, APIs, extension points, [Model Experience](../docs/cookbook/adding-a-package.md#4-write-the-package-readme), and `## Known Limitations and Deferred Work`; both have audited omission allowlists ([Model Experience](../scripts/verify-package-readme-model-experience.ts), [limitations](../scripts/verify-package-readme-limitations.ts)).
