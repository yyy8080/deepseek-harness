...
Picking Linux, Windows, or macOS mints an enrollment and renders the two equivalent ways to start its agent: a copyable one-line command (`curl … | bash`, or `irm … | iex` on Windows) and a download link saving the same generated script as a file. Both fetch the identical script, so the two routes run the same code. The script carries a per-machine key that never expires: the page says so instead of a countdown when the host reports the download open-ended, because the machine reconnects with the same key after going offline. Below them the page lists the machines that have dialled in, each with its status and a permanent "reconnect" marker, and for an attached one its connector id and served working directory, next to a control that revokes the enrollment and disconnects its agent.

An attached machine carries the two actions that make it usable. **Test** completes a real round trip over its live link and reports the latency and the working directory the target itself resolved, or the portal's own actionable message when nothing answers; it re-reads the ledger afterwards, because a machine that stopped answering is no longer the "Connected" the row above it claims. **Start a chat here** creates a conversation bound to that connector, composed from the preset the portal named, with the target's working directory as its cwd, selects it, and closes Settings so the user lands on it; a refusal keeps the panel open on the row that explains it. That action appears only where a conversation flow is mounted and the portal reported the deployment able to bind a session; where it is mounted but the deployment cannot, the row says why instead of offering a button that would silently run on the harness machine.
