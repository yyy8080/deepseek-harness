...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/instance/instance': { kind: 'none', reason: 'The seam owns instance identity and lifecycle; the harness composed inside an instance owns every model-facing registration.' },
  'packages/instance/instance-local-process': { kind: 'none', reason: 'The provider supervises child harness processes; the worker bundle each one boots owns every model-facing registration.' },
  'packages/instance/instance-gateway': { kind: 'none', reason: 'The gateway routes already-composed API messages between runtimes; the instance a conversation lands in owns every model-facing registration.' },
