...
Package tests cover all four packages at full statement, branch, and function coverage on `src`, using real pnpm against temporary profile directories rather than a stubbed package manager. `apps/cli/tests/marketplace-install.e2e.ts` pins the whole flow through the built CLI: pack the sample, search, show, install into a throwaway `DSH_HOME`, assert the profile manifest's `dsh.profile.bundles` and `dsh.marketplace.installs`, assert the installed layer appears in `--dump-config`, uninstall, and assert it is gone. The reject path is covered at both ends: an invalid `dsh.plugin` section fails `parsePluginSection` loud, and an invalid or wrong-version index document fails the static provider with the index path named.
...
The settings **Marketplace** tab (a Host Remote over `ctx.pluginRegistry` plus `plugin-install`, and a `settings.plugins.tab` occupant beside the plugin inventory tab) is not in this change: it needs a new Typert Remote namespace, its generated artifacts, a client package with its own dictionaries, a web-app bundle row, and a web snapshot, and the install operation it would expose is a long-running package-manager run that the Remote layer has no streaming-progress form for. The command line is the complete Consumer for now, and the `--dump-config` transcript in `apps/cli/tests/marketplace-install.e2e.ts` is the assembled-application evidence for the flow. Signing, hash verification, a publish pipeline, and an online plugin editor are likewise out of scope.
