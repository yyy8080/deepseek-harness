...
`connector-execution-world` headless 快照启动出厂的一次性应用，并把它的执行世界放在一个 connector 上，锁定组装出的 transcript：请求中的目标块、经由 connector 操作集路由的 bash 调用，以及回答中来自目标的文件内容。它运行进程内 host，因为远程目标应答同一套操作集，transcript 无法区分二者。
