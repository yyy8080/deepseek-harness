...
flowchart LR
  ...
  svc_instances["ctx.instances<br/>Isolated runtime registry"]
  pkg_instance_local_process["instance-local-process"]
  pkg_instance_gateway["instance-gateway"]
  svc_instanceGateway["ctx.instanceGateway<br/>Multiplexing API gateway over instances"]
  pkg_web["web"]
  ...
  pkg_goal --> svc_goals
  pkg_instance --> svc_instances
  pkg_instance_gateway --> svc_instanceGateway
  pkg_instance_local_process --> svc_instances
  ...
  svc_fs --> pkg_tool_fs
  svc_instances --> pkg_instance_gateway
...
| `ctx.instances` | `seam` | [`instance`](../packages/instance/instance) | [`instance-local-process`](../packages/instance/instance-local-process) | [`instance-gateway`](../packages/instance/instance-gateway) | - | 注册表拥有实例身份与期望/观测状态机；instance-local-process 以拥有自己 DSH_HOME 的子 harness 形式隔离一个运行时，而网关把会话放置到它们之上。 |
| `ctx.instanceGateway` | `core` | [`instance-gateway`](../packages/instance/instance-gateway) | - | - | - | 替代单运行时网关提供 ctx.apiProxy：承载会话的域路由到拥有该会话的实例，宿主平面在本地回答，两条事件流合并。 |
