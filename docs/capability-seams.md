...
| `ctx.instances` | `seam` | [`instance`](../packages/instance/instance) | [`instance-local-process`](../packages/instance/instance-local-process) | [`instance-gateway`](../packages/instance/instance-gateway) | - | The registry owns instance identity and the desired/observed state machine; instance-local-process isolates a runtime as a child harness with its own DSH_HOME, and the gateway places conversations on them. |
| `ctx.instanceGateway` | `core` | [`instance-gateway`](../packages/instance/instance-gateway) | - | - | - | Provides ctx.apiProxy in place of the single-runtime gateway: session-bearing domains route to the instance that owns the session, the host plane answers locally, and the event streams merge. |
